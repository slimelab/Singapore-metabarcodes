...
```{r}
  otu_table_prok <- otu_table %>% filter(taxo0 %in% c("Bacteria", "Archaea"))
  
  otu_mat<- otu_table_prok %>%  select(otu=otu_id,matches("EC|PR|RM|SBW|STJ") )
  
  tax_mat<- otu_table_prok %>%  select(otu=otu_id,taxo0:taxo6) %>% 
...
```{r treemap, fig.height=8, fig.width=8}
  ...
  long_euk_prok <- bind_rows(long_euk, long_prok)
  array_treemap[[label]] <- treemap_gg_dv(filter(long_euk_prok, strait == one_strait), 
...
## Time series by Date (abundant OTUs) aggregated for Singapore strait only
...
## Time series by Date and Station (abundant OTUs) - Division level
...
## Time series by Date and Station - Synechococcales vs Mamiellophyceae

```{r bar_plot_date_syn, fig.height=6, fig.width=8, message=FALSE, warning=FALSE}
  long_syn_mam <- long_euk_prok %>% 
    filter((class == "Synechococcales" & str_detect(genus, "Synechococcus")) | class =="Mamiellophyceae") %>%
...
## Time series by Date and Station (abundant OTUs) - Genus level for Chlorophyta
...
for (one_division in c("Chlorophyta", "Dinoflagellata", "Ochrophyta", "Cryptophyta", "Haptophyta")){
  ...
  print(p[[one_division]])  
   ...
   ggsave("../fig/Fig_Species_Moonsoon.png", grid_fig, height = 12, width = 8, dpi = 300)
```
...
ps_do_nmds <- function(ps_list) {
    ...
    for (i in 1:3) {
      ...
      ps_nmds <- subset_samples(ps_nmds, strait != "Raffles Mari")
...
```{r}
   ...
   ggsave("../fig/Fig_NMDS.png", grid_fig, height = 12, width = 18, dpi = 300)
```
