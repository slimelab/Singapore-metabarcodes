...
```{r temp_per_station, fig.height=10, fig.width=8}
 p <- ggplot(filter(metadata_table, location_label != "BL")) + 
...
```{r rain_per_station, fig.height=10, fig.width=8}
 p <- ggplot(filter(metadata_table, location_label != "BL")) + 
  geom_col(aes(x=date,y=rain, fill=monsoon)) + 
  facet_grid(rows = vars(location)) +
  xlim(as.POSIXct(as.Date(c("2017-01-01","2019-01-01" )))) + 
  geom_vline(xintercept = as.POSIXct(as.Date(c("2017-01-01","2018-01-01", "2019-01-01")))) +
...
```{r rain_mean_per_station}
 ...
 p <- ggplot(rain) + 
  geom_point(aes(x=date,y=rain_mean, color=monsoon), size=5) +
  xlim(as.POSIXct(as.Date(c("2017-01-01","2019-01-01" )))) + 
  geom_vline(xintercept = as.POSIXct(as.Date(c("2017-01-01","2018-01-01", "2019-01-01")))) +
  scale_color_viridis(discrete=TRUE) 
 p
...
```{r bar_plot_genera, fig.height=10, fig.width=8}
for (one_division in c("Chlorophyta", "Dinoflagellata", "Ochrophyta")){
  ...
  p <- plot_bar(ps_subset, x="genus") + 
    facet_grid(rows = vars(monsoon), cols=vars(strait)) +
    geom_col() +
    theme(axis.text.x = element_text(angle = 90, vjust=0.5, hjust=1)) +
