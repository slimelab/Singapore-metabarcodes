...
```{r}
  ...
  sprintf("Euk Photo - all")  
  singa_photo
  
  sprintf("Euk Hetero - all")    
  singa_hetero
```
...
```{r}
  ...
  sprintf("Euk Photo - abundant")  
  singa_photo_abundant
  
  sprintf("Euk Hetero - abundant")    
...
for (i in 1:3) {
  p <- plot_bar(ps_list$ps[[i]], fill = "division") + 
...
for (i in 1:3) {
  p <- plot_bar(ps_list_abundant$ps[[i]], fill = "class") + 
...
### Main genera for different division of EUkaryote Autotrophs
...
### Main species of Mamiellophyceae and Diatoms (Eukaryotes)
