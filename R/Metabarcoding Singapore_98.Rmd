---
title: "Metabarcoding Singapore <br> Euks at 98%"
...
* Assign and analyze eukaryotes for Singapore metabarcoding data (98%)
...
```{r}
 ...
 write_tsv(otu_table_euk_final, "../qiime/otu_table_98_euk_final.tsv")
```
...
```{r}
  otu_table_euk_final <- otu_table_euk_final %>% filter(supergroup_boot > 90)
...
for (i in 1:2) {
  ...
  p <- plot_heatmap(ps_list$ps[[i]], method = "NMDS", distance = "bray", 
               ...
               title=str_c(ps_list$title[[i]], " - Only abundant OTUs (> 10% in at least one sample"))
  print(p)
}
