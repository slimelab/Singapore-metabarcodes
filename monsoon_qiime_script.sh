...
# Put all your reads in the directory specified by starting files location
# Run the script from there with 'bash monsoon_qiime_script.sh', all analyses will be performed in that directory
...
# Unquote below if rrads are zipped
# Put all your reads in the directory raw_reads_combined using mv
# And unzip
...
cd $input/training-feature-classifiers
...
qiime tools export dada2_output_final/table_filt.qza --output-dir dada2_output_exported
qiime tools export dada2_output_final/rep_seqs_filt.qza --output-dir dada2_output_exported
# Also export classification.qza and copy taxa_barplot.qzv and rarefaction_curves.qzv
...
qiime tools export dada2_output_final/table_filt_rarefied.qza --output-dir dada2_output_exported   
qiime tools export dada2_output_final/rep_seqs_filt_rarefied.qza --output-dir dada2_output_exported
...
qiime feature-table filter-samples --i-table ../dada2_output_final/table_filt.qza --o-filtered-table SingStrait.qza --p-where "Strait='SING' AND (Monsoon='SW' OR  Monsoon='NE')" --m-metadata-file ../metadata_gneiss.txt 
qiime feature-table filter-samples --i-table ../dada2_output_final/table_filt.qza --o-filtered-table JohorStrait.qza --p-where "Strait='JOHOR' AND (Monsoon='SW' OR  Monsoon='NE')" --m-metadata-file ../metadata_gneiss.txt 
# add pseudocount and hierarchy for the entire table and for the subsampled tables
qiime gneiss add-pseudocount --i-table ../dada2_output_final/table_filt.qza --p-pseudocount 1 --o-composition-table composition.qza
qiime gneiss correlation-clustering --i-table composition.qza --o-clustering hierarchy.qza
qiime gneiss add-pseudocount --i-table SingStrait.qza --p-pseudocount 1 --o-composition-table compositionSingStrait.qza
qiime gneiss add-pseudocount --i-table JohorStrait.qza --p-pseudocount 1 --o-composition-table compositionJohorStrait.qza
qiime gneiss correlation-clustering --i-table compositionSingStrait.qza --o-clustering hierarchySingStrait.qza
qiime gneiss correlation-clustering --i-table compositionJohorStrait.qza --o-clustering hierarchyJohorStrait.qza

# Visualize the tree hierarchy
qiime gneiss dendrogram-heatmap --i-table composition.qza --i-tree hierarchy.qza --m-metadata-file ../metadata_gneiss.txt --m-metadata-column Strait --p-color-map seismic --o-visualization tree_heatmap_by_Strait.qzv
qiime gneiss dendrogram-heatmap --i-table compositionSingStrait.qza --i-tree hierarchySingStrait.qza --m-metadata-file ../metadata_gneiss.txt --m-metadata-column Monsoon --p-color-map seismic --o-visualization tree_heatmap_Singapore_by_Monsoon.qzv
qiime gneiss dendrogram-heatmap --i-table compositionJohorStrait.qza --i-tree hierarchyJohorStrait.qza --m-metadata-file ../metadata_gneiss.txt --m-metadata-column Monsoon --p-color-map seismic --o-visualization tree_heatmap_Johor_by_Monsoon.qzv

# Calculate the ilr transforms and the regression models
qiime gneiss ilr-transform --i-table composition.qza --i-tree hierarchy.qza --o-balances balances.qza
qiime gneiss ilr-transform --i-table compositionSingStrait.qza --i-tree hierarchySingStrait.qza --o-balances balancesSingStrait.qza
qiime gneiss ilr-transform --i-table compositionJohorStrait.qza --i-tree hierarchyJohorStrait.qza --o-balances balancesJohorStrait.qza

qiime gneiss ols-regression --p-formula "Strait+Monsoon" --i-table balances.qza --i-tree hierarchy.qza --m-metadata-file ../metadata_gneiss.txt --o-visualization regression_summary.qzv
qiime gneiss ols-regression --p-formula "Site+Monsoon" --i-table balancesSingStrait.qza --i-tree hierarchySingStrait.qza --m-metadata-file ../metadata_gneiss.txt --o-visualization regression_summarySingStrait.qzv
qiime gneiss ols-regression --p-formula "Site+Monsoon" --i-table balancesJohorStrait.qza --i-tree hierarchyJohorStrait.qza --m-metadata-file ../metadata_gneiss.txt --o-visualization regression_summaryJohorStrait.qzv

# Balance tax summary view
qiime gneiss balance-taxonomy --i-table composition.qza --i-tree hierarchy.qza \
--i-taxonomy ../taxa_final/classification.qza --p-taxa-level 5 --p-balance-name 'y0' --m-metadata-file ../metadata_gneiss.txt \
--m-metadata-column Strait --o-visualization y0_taxa_summary.qzv

qiime gneiss balance-taxonomy --i-table compositionSingStrait.qza --i-tree hierarchySingStrait.qza \
--i-taxonomy ../taxa_final/classification.qza --p-taxa-level 5 --p-balance-name 'y0' --m-metadata-file ../metadata_gneiss.txt \
--m-metadata-column Monsoon --o-visualization y0_taxa_summarySingStrait.qzv

qiime gneiss balance-taxonomy --i-table compositionJohorStrait.qza --i-tree hierarchyJohorStrait.qza \
--i-taxonomy ../taxa_final/classification.qza --p-taxa-level 5 --p-balance-name 'y0' --m-metadata-file ../metadata_gneiss.txt \
...
# LONGITUDINAL VOLATILITY analysis using sites as different "patients"
mkdir Volatility_analysis
cd Volatility_analysis

# Also looking at the rate of first change. But first I eliminated Raffles samples and re-did Bray Curtis distance
qiime feature-table filter-samples --i-table ../dada2_output_final/table_filt.qza --o-filtered-table table_filt_noraffles.qza --p-where "Strait='Singapore' OR Strait='Johor'" --m-metadata-file ../metadata_Longitudinal.txt
...
qiime longitudinal first-distances --i-distance-matrix diversity_noraffles/bray_curtis_distance_matrix.qza --m-metadata-file ../metadata_Longitudinal.txt --p-state-column Day_number --p-individual-id-column Site --p-replicate-handling random --o-first-distances first-distances_BC.qza

qiime longitudinal volatility --m-metadata-file first-distances_BC.qza --m-metadata-file ../metadata_Longitudinal.txt --p-state-column Day_number --p-default-metric Distance --output-dir Volatility_results2
